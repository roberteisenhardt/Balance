...
public var walkMaxAnimationSpeed : float = 0.9;
public var trotMaxAnimationSpeed : float = 1.3;
public var runMaxAnimationSpeed : float = 0.9;
...
var zoomingSpeed = 2.5f;
// added by xinyi ends here
...
private var gravityFactor = 1.0f;
...
private var moveSpeedfactor = 1.0f;
...
// added by Xinyi Starts here
private var cameraScript : ThirdPersonCamera;

private var isZooming = false;

private var targetHeight;

private var targetDistance;
// added by Xinyi Ends here

// Rob's work starts here
...
function UpdateSmoothedMovementDirection ()
{
	...
	if(Mathf.Abs (h) == 1 || (Mathf.Abs (h) == 1 && Mathf.Abs (v) == 1)){
		movingBack = false;
	}
	else if( v == -1){
		movingBack = true;
	...
	isMoving = Mathf.Abs (h) == 1 || Mathf.Abs (v) == 1;
	...
	if (grounded)
	{
		...
		moveSpeed = Mathf.Lerp(moveSpeed, targetSpeed, curSmooth) * moveSpeedfactor;
	...
	else
	{
		...
		if (isMoving){
			inAirVelocity += moveDirection.normalized * Time.deltaTime * inAirControlAcceleration;
		}
	}
...
function ApplyGravity ()
{
	if (isControllable)	// don't move player at all if not controllable.
	{
		...
		if (IsGrounded ())
			...
			verticalSpeed -= gravity * Time.deltaTime * gravityFactor;
	}
}
...
function CalculateJumpVerticalSpeed (targetJumpHeight : float)
{
	...
	return Mathf.Sqrt(2 * targetJumpHeight * gravity * gravityFactor);
}
...
function UpdatePosition(newPosition : Vector3){
	transform.position = newPosition;
}

function adjustCharacterSpeed(newSpeed : float){
	gravityFactor = newSpeed/2;
	moveSpeedfactor = newSpeed;
}

function resetCharacterSpeed(){
	gravityFactor = 1;
	moveSpeedfactor = 1;
}
